...
import { getItemGroup } from "./graphql/queries";
import { updateItemGroup } from "./graphql/mutations";
...
export default function ItemGroupUpdateForm(props) {
  const {
    id: idProp,
    itemGroup: itemGroupModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = itemGroupRecord
      ? { ...initialValues, ...itemGroupRecord }
      : initialValues;
  ...
  const [itemGroupRecord, setItemGroupRecord] =
    React.useState(itemGroupModelProp);
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? (
            await client.graphql({
              query: getItemGroup.replaceAll("__typename", ""),
              variables: { id: idProp },
            })
          )?.data?.getItemGroup
        : itemGroupModelProp;
      setItemGroupRecord(record);
    };
    queryData();
  }, [idProp, itemGroupModelProp]);
  React.useEffect(resetStateValues, [itemGroupRecord]);
    ...
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await client.graphql({
            query: updateItemGroup.replaceAll("__typename", ""),
            variables: {
              input: {
                id: itemGroupRecord.id,
                ...modelFields,
              },
            },
          });
      ...
      {...getOverrideProps(overrides, "ItemGroupUpdateForm")}
      {...rest}
    >
      <TextField
        label="Quantity"
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || itemGroupModelProp)}
